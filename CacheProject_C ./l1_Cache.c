                     ...
                     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  for(int i=0; i<L1_LINES_PER_SET;i++){
    if((l1_cache[l1_set_index].lines[i].v_r_d_tag&L1_VBIT_MASK)&&((l1_cache[l1_set_index].lines[i].v_r_d_tag&L1_ENTRY_TAG_MASK)==(l1_address_tag))){//error?
      // printf("wow2\n" );
      l1_cache[l1_set_index].lines[i].v_r_d_tag|=L1_RBIT_MASK;
      *status|=L1_CACHE_HIT_MASK;
      if(control&READ_ENABLE_MASK){
      *read_data=l1_cache[l1_set_index].lines[i].cache_line[l1_word_offset];

      }
      if(control & WRITE_ENABLE_MASK){
        l1_cache[l1_set_index].lines[i].cache_line[l1_word_offset]=write_data;
          l1_cache[l1_set_index].lines[i].v_r_d_tag|=L1_DIRTYBIT_MASK;
                    ...
                    uint8_t *status)
{
 ...
 uint32_t choose;
  ...
  for(int i=0; i<L1_LINES_PER_SET; i++){
	...
	l1_cache[l1_set_index].lines[i].cache_line[word_offset] = write_data[word_offset];
      }

        l1_cache[l1_set_index].lines[i].v_r_d_tag|=L1_VBIT_MASK;
        l1_cache[l1_set_index].lines[i].v_r_d_tag&=~L1_DIRTYBIT_MASK;
        l1_cache[l1_set_index].lines[i].v_r_d_tag&=~L1_RBIT_MASK;
        l1_cache[l1_set_index].lines[i].v_r_d_tag&=~L1_ENTRY_TAG_MASK;
        l1_cache[l1_set_index].lines[i].v_r_d_tag|=l1_address_tag;
        *status&=~L1_CACHE_HIT_MASK;
        return;

    }

   if((!(l1_cache[l1_set_index].lines[i].v_r_d_tag&L1_DIRTYBIT_MASK)) && (!(l1_cache[l1_set_index].lines[i].v_r_d_tag&L1_RBIT_MASK))){
    if(r0_d0_index==~0x0) {
      r0_d0_index = i;
      }
    }
  else if(((l1_cache[l1_set_index].lines[i].v_r_d_tag&L1_DIRTYBIT_MASK))&& (!(l1_cache[l1_set_index].lines[i].v_r_d_tag&L1_RBIT_MASK))){
    if (r0_d1_index == ~0x0) {
      r0_d1_index = i;
      }
  ...
  if (r0_d0_index != ~0x0){
    ...
    choose = 0;  //they all have r = 1 and d = 1, so just pick 0th element.
  }

if(l1_cache[l1_set_index].lines[choose].v_r_d_tag&L1_DIRTYBIT_MASK){
  *evicted_writeback_address=((l1_cache[l1_set_index].lines[choose].v_r_d_tag&L1_ENTRY_TAG_MASK) <<L1_ADDRESS_TAG_SHIFT)| (l1_set_index<<L1_SET_INDEX_SHIFT);
  *status|=L1_CACHE_HIT_MASK;

  for(uint32_t word_offset=0;word_offset<WORDS_PER_CACHE_LINE;word_offset++) {
evicted_writeback_data[word_offset]=l1_cache[l1_set_index].lines[choose].cache_line[word_offset];
  }
...
l1_cache[l1_set_index].lines[choose].cache_line[word_offset] = write_data[word_offset];
  ...
  l1_cache[l1_set_index].lines[choose].v_r_d_tag|=L1_VBIT_MASK;
  l1_cache[l1_set_index].lines[choose].v_r_d_tag&=~L1_DIRTYBIT_MASK;
  l1_cache[l1_set_index].lines[choose].v_r_d_tag&=~L1_ENTRY_TAG_MASK;//needed?
  l1_cache[l1_set_index].lines[choose].v_r_d_tag|=l1_address_tag;
