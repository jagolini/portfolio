                   ...
                   uint8_t control, uint32_t *read_data)
{
  ...
  l1_cache_access(address, write_data,control,read_data,&status);// &status?
...
void memory_handle_l1_miss(uint32_t address)
{
  ...
  uint8_t status;
  //
  l2_cache_access( address, write_data, READ_ENABLE_MASK,  read_data, &status);
  ...
  //
  if((status & 0x1F)){
  l2_cache_access(evicted_writeback_address,evicted_writeback_data, WRITE_ENABLE_MASK, read_data,&status);
  if( !(status & 0x1F)){
...
void memory_handle_l2_miss(uint32_t address, uint8_t control)
{
  ...
  //
  if((status & 0x1F)){
    main_memory_access(evicted_writeback_address, evicted_writeback_data,WRITE_ENABLE_MASK, read_data);
  }
}
