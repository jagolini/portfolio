...
void l2_initialize()
{
  ...
  l2_cache[i].v_d_tag =0;// i could be wrong might need i>>2 or smthg
}
                     ...
                     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  //
  uint32_t l2index= (address & L2_INDEX_MASK)>>L2_INDEX_SHIFT;
  ...
  //
  uint32_t l2tag= (address & L2_ADDRESS_TAG_MASK)>>L2_ADDRESS_TAG_SHIFT;
...
if((!(l2_cache[l2index].v_d_tag & L2_VBIT_MASK))||((l2_cache[l2index].v_d_tag & L2_ENTRY_TAG_MASK)!=l2tag)){
  *status&=~L2_HIT_STATUS_MASK;
  ...
  if(control & READ_ENABLE_MASK){
    void* destination =read_data;
    const void* source=l2_cache+(l2index);

  for(int offset=0; offset < WORDS_PER_CACHE_LINE; offset++){
    read_data[offset]=l2_cache[l2index].cache_line[offset];
  }
  }
  if(control & WRITE_ENABLE_MASK){
    void* destination =l2_cache+(l2index);
    const void* source=write_data;

for(int offset=0; offset < WORDS_PER_CACHE_LINE; offset++){
  l2_cache[l2index].cache_line[offset]=write_data[offset];
}
  l2_cache[l2index].v_d_tag|=L2_DIRTYBIT_MASK;
  }
                    ...
                    uint8_t *status)
{
  ...
  if((!(l2_cache[l2index].v_d_tag & L2_VBIT_MASK))||(!(l2_cache[l2index].v_d_tag & L2_DIRTYBIT_MASK))){
    void* destination=l2_cache+(l2index);
    const void* source=write_data;
...
for(int offset=0; offset < WORDS_PER_CACHE_LINE; offset++){
  ...
  l2_cache[l2index].v_d_tag |= L2_VBIT_MASK;
  l2_cache[l2index].v_d_tag &= ~L2_DIRTYBIT_MASK;
  l2_cache[l2index].v_d_tag &= ~L2_ENTRY_TAG_MASK;
  l2_cache[l2index].v_d_tag |= l2tag;
  *status&=~L2_HIT_STATUS_MASK;
  return;
  }
...
//}
  ...
  *evicted_writeback_address=((l2_cache[l2index].v_d_tag & L2_ENTRY_TAG_MASK) << L2_ADDRESS_TAG_SHIFT) | (l2index << L2_INDEX_SHIFT);
  void* destination1=evicted_writeback_data;
  const void* source1=l2_cache+(l2index);

for(int offset=0; offset < WORDS_PER_CACHE_LINE; offset++){
  evicted_writeback_data[offset]=l2_cache[l2index].cache_line[offset];
}
*status|=L2_HIT_STATUS_MASK;

  void* destination2=l2_cache+(l2index);
  const void* source2=write_data;
