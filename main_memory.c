...
void main_memory_initialize(uint32_t size_in_bytes)
{
  ...
  //CODE HERE
  for (unsigned int i = 0; i < size_in_bytes>>2; i++) {
    main_memory[i]=0;
  }
...

{
 ...
 printf("Error: The address needs to be within the size of the memory\n");
exit(1);
}
  //CODE HERE
...
uint32_t wordstart=((address & CACHE_LINE_ADDRESS_MASK)>>2);//
  //CODE HERE
...
if(control & READ_ENABLE_MASK){
  ...
  const void* source=main_memory+(wordstart);
...
if(control & WRITE_ENABLE_MASK){
  ...
  void* destination =main_memory+(wordstart);
